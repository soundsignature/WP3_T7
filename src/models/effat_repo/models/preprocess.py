...
class AugmentMelSTFTv2(nn.Module):  # Works better with 48kHz
    def __init__(self, sr=32000, n_mels=128, n_fft=4096, hopsize=1024, win_length=4096, 
                 fmin=50.0, fmax=None, fmin_aug_range=5, fmax_aug_range=1000, freqm=48, timem=192):
        super().__init__()
        self.sr = sr
        self.n_mels = n_mels
        self.n_fft = n_fft
        ...
        assert fmin_aug_range >= 1, "fmin_aug_range should be >=1; 1 means no augmentation"
        assert fmax_aug_range >= 1, "fmax_aug_range should be >=1; 1 means no augmentation"
    ...
    def forward(self, x):
        ...
        x = torch.stft(x, self.n_fft, hop_length=self.hopsize, win_length=self.win_length, window=self.window, return_complex=False)
        x = (x ** 2).sum(dim=-1) ** 0.5  # power mag

        if self.training:
            fmin = max(0, min(self.fmin + torch.randint(-self.fmin_aug_range, self.fmin_aug_range, (1,)).item(), self.sr // 2))
            fmax = min(self.sr // 2, max(self.fmax + torch.randint(-self.fmax_aug_range // 2, self.fmax_aug_range // 2, (1,)).item(), fmin + 1))
        else:
            fmin = self.fmin
            fmax = self.fmax
